				...
				<div class="cust-left-block py-0 py-lg-4">					
									...
									<div class="login-input mt-4">
										...
										<div class="d-flex mt-3 align-items-center">
											...
											@if(isset(user()->profile->cover_image))
												...
												<img src="{{ asset('public/images/rider/cover_images')}}/{{(isset(user()->profile->cover_image) && (!empty(user()->profile->cover_image))) ? user()->profile->cover_image : 'rider.jpg'}}" class="img-fluid rounded-lg" >
											...
											<div class="dropzone flex-row p-3 justify-content-start position-relative mt-3 h-75" id="dropzone">
												...
												<div class="drop-box-format  ">Drag and drop <span class="text-gray">or</span> <span class="text-danger">Select from Gallery</span></div>
												 <input type="file" class="input-block upload-group-detail h-100" name="cover_image" placeholder="">
					...
					<div class="row ">
						...
						<div class="col-md-4">
							...
							<p class="about-content">{{substr($description, 0, 250)}}..</p>
					...
					<div class="d-flex align-items-center filter-details mb-4"> <span class="filter-block1">Explore my recent and past trips.</span> 
					@if(count($rides) > 0)
					<span class="ml-auto filter-block3 mob-filter"><a href="{{route('my-rides')}}">View all Rides</a></span> 
					@endif
					</div>
					...
					<div class="row">
						...
						<div class="col-12 mb-3 bike-refferer-{{$bike->id}}">
							...
							<div class="rides-block d-none d-md-flex">
								...
								<div class="bike-img-block  mr-md-3 ml-3 ml-md-0 order-2 order-md-1">

									@if(!empty($bike->image))

									<img src="{{ asset('public/images/rider/bikes/')}}/{{isset(json_decode($bike->image,true)[0]) ? json_decode($bike->image,true)[0] : 'not_found.jpg'  }}" >
								...
								<div class="rider-details-block w-100 order-1 order-md-2 box--half">
									...
									@if(!empty($bike->info))
									<div class="d-flex align-items-center location-block"> 
										<span class="time font-italic">Review Highlight </span> 
									</div>
									<p class="prof-byke-detail"> {{substr($bike->info, 0, 200)}}..</p>
									@endif
